...
let loadMoreDivTop = document.querySelector(".load-more-div-top");
let loadMoreBtnTop = document.querySelector(".load-more-btn-top");

getIdList(process.env.API_URL_TOP)
  .then(() => getNextDetails())
  .then((details) => createNextCards(details))
  .then(() => {
    changeOpacity(loadMoreBtnTop, "1", 1000);
  })
  .catch((e) => {
...
loadMoreBtnTop.addEventListener("click", function () {
  startLoading();
  loadMoreBtnTop.setAttribute("disabled", "true");
  getNextDetails()
    .then((details) => createNextCards(details))
    .catch((e) => {
      let modalBody = document.querySelector(".modal-body");
    ...
    .finally(() => {
      stopLoading();
      loadMoreBtnTop.removeAttribute("disabled");
    });
});
